    ...
    if usuario and usuario.verificar_senha(data['senha']):
        ...
        if not usuario.ativo:
            return jsonify({
                ...
                'usuario': usuario.as_dict()  # Opcional: incluir dados do usuário
            }), 403  # 403 Forbidden - Acesso negado
        
        # Cria a sessão do usuário
        session['usuario_id'] = str(usuario.id)
        session['usuario_nome'] = usuario.usuario
        session['tipo_usuario'] = usuario.tipo
        session['usuario_ativo'] = usuario.ativo  # Armazena status na sessão
        
        return jsonify({
            'message': 'Login bem-sucedido',
            'usuario': usuario.as_dict(),
            'redirect': '/inicial.html'
        }), 200
